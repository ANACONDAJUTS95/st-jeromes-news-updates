            ...
            <div className="mb-10 md:mb-12">
              ...
              <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 md:gap-8 pb-10 md:pb-12">
                <div className="max-w-3xl space-y-4">
                  <p className="text-label-caps text-secondary">{article.category}</p>
                  <h1 className="text-headline-lg">{article.title}</h1>
                  <p className="text-lg md:text-xl italic font-serif text-on-surface-muted">{article.subtitle}</p>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16">
              <div className="lg:col-span-8 space-y-10 md:space-y-12">
                <div className="relative aspect-video bg-surface rounded-sm overflow-hidden shadow-academic">
                  <Image src={article.image} alt={article.title} fill className="object-contain p-12 md:p-20 opacity-20" />
                </div>

                <div className="flex items-center justify-between py-4 md:py-6">
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 rounded-full bg-secondary/15 flex items-center justify-center text-primary flex-shrink-0">
                      {article.author[0]}
                    </div>
                    <div>
                      <p className="text-label-sm">{article.author}</p>
                      <p className="text-label-sm text-on-surface-muted">{article.date} &bull; {article.readingTime}</p>
                    </div>
                  </div>
                </div>
          ...
          <div className="fixed inset-0 z-50 overflow-y-auto pb-40">
            ...
            <div className={`max-w-3xl mx-auto px-4 md:px-6 py-16 md:py-24 lg:py-32 transition-all duration-500`} style={{ fontSize: `${fontSize}px`, lineHeight: 1.6, fontWeight: fontWeight }}>
              <div className="mb-12 md:mb-16 space-y-4">
                <p className="text-label-caps text-secondary text-center">{article.category}</p>
